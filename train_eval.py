...
def train_multiple_epochs(train_dataset,
                          test_dataset, train_u_indices, test_u_indices,
                          model,
                          args,
                          lr_decay_factor,
                          lr_decay_step_size,
...
def train(model, optimizer, loader, device, regression=False, ARR=0):
    ...
    for data in loader:
        optimizer.zero_grad()
        data = data.to(device)  # Batch(batch=[2167], edge_attr=[5446, 1], edge_index=[2, 5446], edge_type=[5446], x=[2167, 4], y=[50])
        out = model(data)  # shape torch.Size([50])
        if regression:
