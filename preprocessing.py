...
'''
def load_data_monti(dataset, testing=False, rating_map=None, post_rating_map=None, metatest_users=100):
    """
    ...
    print("old train/test:", num_train+num_val, "/", num_test, "old ratings:", num_train+num_test+num_val)
    ...
    # all
    # new split here by users
    idx_nonzero = np.concatenate([idx_nonzero_train, idx_nonzero_test], axis=0)
    pairs_nonzero = np.concatenate([pairs_nonzero_train, pairs_nonzero_test], axis=0)

    train_idx = idx_nonzero[0:num_train + num_val]
    test_idx = idx_nonzero[num_train + num_val:]

    assert(len(test_idx) == num_test)

    train_pairs_idx = pairs_nonzero[0:num_train + num_val]
    test_pairs_idx = pairs_nonzero[num_train + num_val:]

    # ranking training
    pairs1 = np.argsort(train_pairs_idx, axis=0)
    listidx = list()
    listpairs = list()
    for i in range(len(train_idx)):
        index = pairs1[i][0]
        listidx.append(train_idx[index])
        listpairs.append(train_pairs_idx[index])

    idx_nonzeronew = np.array(listidx)
    pairs_nonzeronew = np.array(listpairs)
    ignore_keys, ignores, afterig_users = user_itemnum(pairs_nonzeronew)
    print("ignores users:", len(ignore_keys), "ignores ratings:",ignores, "split meta_test_users:", metatest_users)

    # ignores
    listidxig = list()
    listpairsig = list()
    for i in range(len(train_idx)):
        if pairs_nonzeronew[i][0] not in ignore_keys:
            listidxig.append(idx_nonzeronew[i])
            listpairsig.append(pairs_nonzeronew[i])

    idx_nonzeroig = np.array(listidxig)
    pairs_nonzeroig = np.array(listpairsig)

    # idx_nonzero = idx_nonzeroig  # 23217
    # pairs_nonzero = pairs_nonzeroig  # 23217

    # no need split meta-test
    # select_test = random.sample(afterig_users, metatest_users)
    # print(select_test)
    train_idx = list()
    train_pairs_idx = list()
    for i in range(len(idx_nonzeroig)):
        if pairs_nonzeroig[i][0] in afterig_users:
            train_idx.append(idx_nonzeroig[i])
            train_pairs_idx.append(pairs_nonzeroig[i])

    # print("select meta-test users:", select_test)
    print("train ratings nums:", len(train_idx), "test ratings nums:", len(test_idx), "all_ratings:", len(train_idx)+len(test_idx))
    # graph = print_traingraph(test_pairs_idx)
    # exit()

    val_idx = []
    train_idx = np.array(train_idx)  # <class 'tuple'>: (21091,)
    test_idx = np.array(test_idx)  # <class 'tuple'>: (2126,)

    val_pairs_idx = []
    train_pairs_idx = np.array(train_pairs_idx)
    test_pairs_idx = np.array(test_pairs_idx)
...
def user_itemnum(pairs_nonzero):
    g = {}
    for i in range(len(pairs_nonzero)):
        g.setdefault(pairs_nonzero[i][0], []).append(pairs_nonzero[i][1])  # 6380
    # print(g)
    # print(len(g))
    traindict = {k: len(v) for k, v in g.items() if len(v) < 35}
    afterig = {k: len(v) for k, v in g.items() if len(v) > 34}
    # newtraindict = sorted(traindict.items(), key=lambda x: x[1], reverse=True)
    # a1 = dict(newtraindict)
    list_values = [i for i in traindict.values()]
    list_keys = [i for i in traindict.keys()]
    afterig_users = [i for i in afterig.keys()]
    # print(newtraindict)
    return list_keys, sum(list_values), afterig_users
def print_traingraph(pairs_nonzero):
    G = nx.Graph()
    for i in range(len(pairs_nonzero)):
        G.add_edge(pairs_nonzero[i][0], pairs_nonzero[i][1])
    nx.draw_networkx(G)
