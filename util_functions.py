...
class Metaset(InMemoryDataset):
    def __init__(self, data_list, transform=None, pre_transform=None):
        self.data_list = data_list
        # super(Metaset, self).__init__(transform, pre_transform)
    def process(self):
...
def links2subgraphs(
    ...
    return train_graphs, val_graphs, test_graphs  # 23556 [] 2617
...
def dropout_adj(edge_index,
    ...
    从邻接矩阵中随机删除边
    ：obj：`（edge_index，edge_attr）`具有可传播性：obj：`p`使用来自伯努利分布。
    Args:
    ...
    参数：
    边缘指数（LongTensor）：边缘指数。
    edge_attr（张量，可选）：边权重或多维边缘特征。（默认：：obj:`None`）
    p（float，可选）：退出概率。（默认：：obj:`0.5`）
    force_undirected（bool，可选）：如果设置为：obj:`True'，则删除或保留无向边的两条边。
    （默认：：obj:`False`）
    num_nodes（int，可选）：节点数，即*：obj:`max_val+1`共：attr:`edge_index`。（默认：：obj:`None`）
    训练（bool，可选）：如果设置为：obj:`False'，则此操作是无操作（默认：：obj:`True`）
    """
    ...
    if not training:
        return edge_index, edge_attr  # torch.Size([2, 5446]) torch.Size([5446])

    N = num_nodes  # 2167
    row, col = edge_index  # torch.Size([4359])
